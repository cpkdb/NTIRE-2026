...
class Model(nn.Module):
    def __init__(
        ...
        self.clip, self.preprocess = clip.load(backbone[0], device=device, download_root=download_root)
        ...
        if num_hooks is not None and num_hooks < len(all_hooks):
            for h in all_hooks[:-num_hooks]:
                h.close()
            self.hooks = all_hooks[-num_hooks:]
        else:
